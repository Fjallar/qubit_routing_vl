...
from dqn2.torch_layers import NatureCNN
...
class DQNPolicy(BasePolicy):
    ...
    def predict(
        ...
        """
        # print("correct method used!")
        self.set_training_mode(False)
        observations = env.env_method("get_next_observations")
        actions = np.zeros(env.num_envs, dtype = int)

        for i, (obs, action_idxs, rewards,_,_) in enumerate(observations):
            with th.no_grad():
                tensor_obs = th.Tensor(obs).to(self.device)
                value = self._predict(tensor_obs, deterministic=deterministic)
            value = value.reshape((th.numel(value)))
            value_np = value.detach().cpu().numpy()

            value_np+=rewards
            actions[i] = action_idxs[np.argmax(value_np)]
