...
from dqn2.buffers import ReplayBuffer
from dqn2.policies import DQNPolicy, CnnPolicy, MlpPolicy
from dqn2.evaluation import evaluate_policy
...
class DQN2(OffPolicyAlgorithm):
    ...
    def __init__(
        ...
        if type(policy)==str:
            policy = {'MlpPolicy':MlpPolicy, 'CnnPolicy':CnnPolicy, 'DQNPolicy': DQNPolicy}[policy]
    ...
    def predict(
        ...
        """
        env = self.env if env==None else env

        if not deterministic and np.random.rand() < self.exploration_rate:
            # n_batch = observation.shape[0]
            # action = np.array([self.action_space.sample() for _ in range(n_batch)])
            action_masks = env.env_method("get_action_mask")
            action_indecies = [np.argwhere(mask).flatten() for mask in action_masks]
            action = np.array([np.random.choice(indices) for indices in action_indecies])
        else:
    ...
    def learn(
        ...
        while self.num_timesteps < total_timesteps:
            print("timesteps: {}/{}".format(self.num_timesteps, total_timesteps))
    ...
    def _excluded_save_params(self) -> List[str]:
        return super(DQN2, self)._excluded_save_params() + ["q_net", "q_net_target"]
    ...
    def _sample_action(
        ...
        if (self.num_timesteps < self.learning_starts and not (self.use_sde and self.use_sde_at_warmup)):
            ...
            action_masks = env.env_method("get_action_mask")
            action_indecies = [np.argwhere(mask).flatten() for mask in action_masks]
            actions = np.array([np.random.choice(indices) for indices in action_indecies])
            return actions
    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            # print("collected steps: {}".format(num_collected_steps))
